...
<<<<<<< Updated upstream
=======
//Fetch the user data object to be returned.
async function getUserDataObject(validEmail)  {
    const userFetchString = 'http://localhost:8080/api/user?email=' + validEmail;
    const response = await fetch(userFetchString);
>>>>>>> Stashed changes
    return response;
}

export {validateUser, getUserDataObject};